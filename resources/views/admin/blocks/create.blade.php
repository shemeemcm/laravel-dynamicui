...
<div class="container-fluid p-0" style="max-width: 900px;">
    <div class="mb-4">
        ...
        <h2 class="fw-extrabold text-slate-800 mt-2">Add UI Block</h2>
        <p class="text-secondary">Create a new block to render on the client homepage.</p>
    </div>
        ...
        <div class="card-body p-4">
            <form action="{{ route('admin.blocks.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title" value="{{ old('title') }}" placeholder="e.g. Features that Empower You" required>
                    @error('title')
                ...
                <div class="mb-3">
                    ...
                    <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                        ...
                        <option value="banner" {{ old('type') == 'banner' ? 'selected' : '' }}>Banner (Hero Section)</option>
                        <option value="card" {{ old('type') == 'card' ? 'selected' : '' }}>Card (Grid of Cards)</option>
                        <option value="list" {{ old('type') == 'list' ? 'selected' : '' }}>List (Comma Separated Items)</option>
                        <option value="stats" {{ old('type') == 'stats' ? 'selected' : '' }}>Stats (Statistic Grid)</option>
                    </select>
                    @error('type')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                ...
                <div class="card bg-light border border-light-subtle mb-3 d-none" id="content-guide">
                    <div class="card-body">
                        ...
                        <div class="position-relative">
                            ...
                            <button type="button" class="btn btn-sm btn-outline-light position-absolute top-0 end-0 m-2" id="use-template-btn">Use Template</button>
                        </div>
                    </div>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea class="form-control @error('content') is-invalid @enderror" id="content" name="content" rows="8" placeholder="Enter content or structure here..." required>{{ old('content') }}</textarea>
                    @error('content')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('display_order') is-invalid @enderror" id="display_order" name="display_order" value="{{ old('display_order') }}" placeholder="Leave blank to add at the end">
                ...
                <div class="d-flex gap-2">
                    <button type="submit" class="btn btn-primary rounded-3 px-4">Create Block</button>
...
<script>
    $(document).ready(function() {
        ...
        // Populate Old Input type guide if preset
        if($('#type').val()){
            updateGuide();
        }
